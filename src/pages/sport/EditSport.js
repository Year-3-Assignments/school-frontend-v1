...
import { getAllSportStudent, getAllSportCoach, setSport, updateSport } from '../../actions/sportActions';
...
constructor(props) {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    if (this.props.setsport !== nextProps.setsport) {
      ...
      this.setState({ alreadySelectedCoachValue: coachName.map(coach => coach.value) });

      this.setState({ alreadySelectedPlayerhValue: playersName.map(player => player.value) });
    ...
    return (
      <div>
        {this.state.selectedCoach.map((item, index) => {
          return (<div className="d-flex flex-row"> <p style={{marginRight: '0.2em'}}> {item.firstName} {item.lastName} </p> <button onClick={() => this.removeCoach(index)}>x</button> </div>)
        })}
    </div>
      )
    }
    ...
    return (
      <div>
        {this.state.selectedPlayers.map((item, index) => {
          return (<div className="d-flex flex-row"> <p style={{marginRight: '0.2em'}}> {item.firstname} {item.lastname} </p> <button onClick={() => this.removePlayers(index)}>x</button> </div>)
        })}
      </div>
    )
  }
    ...
    if(this.validateResourceForm()){
      ...
      if(!data.includes(false)){
        let resource = {
          ...
          teamPlayers: this.state.alreadySelectedPlayerhValue.concat(this.state.teamPlayers),
        };
      ...
      <div className="modal-dialog modal-lg">
          ...
          <div className="modal-body">
            ...
            <div className="row m-0 mb-3">
              ...
              <Select
                    defaultValue={this.state.coach.length > 0 ? this.state.students[0].label : null}
                    isMulti
                    name="students"
            ...
            <div className="row m-0 mb-3">
              ...
              <Select
                    defaultValue={this.state.students.length > 0 ? this.state.students[0].label : null}
                    isMulti
                    name="students"
